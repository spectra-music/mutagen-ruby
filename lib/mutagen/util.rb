...
# C character buffer to Ruby numeric type conversions
    module CData
      # Convert from
      def self.short_le(data)
        data.unpack('s<')[0];
      end

      def self.ushort_le(data)
        data.unpack('S<')[0];
      end

      def self.short_be(data)
        data.unpack('s>')[0];
      end

      def self.ushort_be(data)
        data.unpack('S>')[0];
      end

      def self.int_le(data)
        data.unpack('i<')[0];
      end

      def self.uint_le(data)
        data.unpack('I<')[0];
      end

      def self.int_be(data)
        data.unpack('i>')[0];
      end

      def self.uint_be(data)
        data.unpack('I>')[0];
      end

      def self.longlong_le(data)
        data.unpack('q<')[0];
      end

      def self.ulonglong_le(data)
        data.unpack('Q<')[0];
      end

      def self.longlong_be(data)
        data.unpack('q>')[0];
      end

      def self.ulonglong_be(data)
        data.unpack('Q>')[0];
      end
      ...
      def self.to_uint_be(data)
        [data].unpack('I>');
      end

      def self.to_longlong_le(data)
        [data].unpack('q<');
      end

      def self.to_ulonglong_le(data)
        [data].unpack('Q<');
      end

      def self.to_longlong_be(data)
        [data].unpack('q>');
      end

      def self.to_ulonglong_be(data)
        [data].unpack('Q>');
      end

      BITSWAP = (0..255).map { |val| (0..7).map { |i| (((val >> i) & 1) << (7-i)) }.inject(:+).chr }.join

      def self.test_bit(value, n)
        ((value >> n) & 1) == 1;
      end
    end
